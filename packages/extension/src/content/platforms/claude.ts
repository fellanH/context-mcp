...
const log = (...args: unknown[]) => console.debug("[context-vault:claude]", ...args);
...
function extractTaggedMessages(
  ...
  for (const el of sorted) {
    ...
    if (content) {
      messages.push({ index: messages.length, role: roleMap.get(el)!, content, platform });
    }
  }
  ...
  getMessages(): ChatMessage[] {
    try {
      ...
      const result1 = extractTaggedMessages(
        Array.from(document.querySelectorAll<HTMLElement>('[data-testid="user-message"]')),
        Array.from(document.querySelectorAll<HTMLElement>('[data-testid="assistant-message"]')),
        this.name
      );
      if (result1) {
      ...
      const result2 = extractTaggedMessages(
        Array.from(document.querySelectorAll<HTMLElement>(".font-user-message")),
        Array.from(document.querySelectorAll<HTMLElement>(".font-claude-message")),
        this.name
      );
      if (result2) {
      ...
      const result3 = extractTaggedMessages(
        Array.from(document.querySelectorAll<HTMLElement>('[data-is-streaming] .human-turn, [data-testid="human-turn"]')),
        Array.from(document.querySelectorAll<HTMLElement>('[data-is-streaming] .ai-turn, [data-testid="ai-turn"]')),
        this.name
      );
      if (result3) {
      ...
      const container = document.querySelector<HTMLElement>('[class*="conversation"]') ||
