...
export class ErrorBoundary extends React.Component<Props, State> {
  ...
  componentDidCatch(error: Error) {
    console.error(`[context-vault:ErrorBoundary${this.props.label ? `:${this.props.label}` : ""}]`, error);
  }
    ...
    if (this.state.hasError) {
      return (
        <div className="p-6 text-center">
          ...
          <div className="text-xs text-muted-foreground mb-4">{this.state.error}</div>
          <button
