...
import { DEFAULT_LIFECYCLE } from "../constants.js";
...
export function findArchiveCandidates(ctx, { dryRun = false } = {}) {
  ...
  for (const [key, rules] of Object.entries(lifecycle)) {
    ...
    const cutoff = new Date(
      now.getTime() - rules.archiveAfterDays * 86400000,
    ).toISOString();
...
export async function archiveEntries(ctx, { dryRun = false } = {}) {
  const candidates = findArchiveCandidates(ctx, { dryRun });
  if (candidates.length === 0 || dryRun) return { archived: candidates, count: candidates.length };
...
export async function restoreEntry(ctx, entryId) {
  ...
  const { parseEntryFromMarkdown } = await import("./frontmatter.js");
  const { categoryFor, defaultTierFor } = await import("./categories.js");
  const { indexEntry } = await import("../index/index.js");

  const relFromVault = relative(vaultDir, destPath);
  const kindDir = relFromVault.split("/").filter(Boolean);
  let kind = meta.kind || (kindDir.length >= 2 ? kindDir[kindDir.length - 2] : kindDir[0]) || "note";
