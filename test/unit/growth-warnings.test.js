...
describe('computeGrowthWarnings', () => {
  ...
  it('warns on event entries exceeding threshold', () => {
    ...
    expect(result.hasWarnings).toBe(true);
    expect(result.warnings.some((w) => w.message.includes('Event entries'))).toBe(true);
  });
  ...
  it('event warning includes TTL note when events lack expires_at', () => {
    ...
    const eventWarning = result.warnings.find((w) => w.message.includes('Event entries'));
  ...
  it('respects custom thresholds', () => {
    ...
    const result = computeGrowthWarnings(makeStatus({ embeddingStatus: { total: 3000 } }), custom);
    expect(result.hasWarnings).toBe(false);
  });
  ...
  it('warns on database size exceeding threshold', () => {
    ...
    expect(result.hasWarnings).toBe(true);
    expect(result.warnings.some((w) => w.message.includes('Database size'))).toBe(true);
  });
  ...
  it('warns on events without TTL exceeding threshold', () => {
    ...
    expect(result.hasWarnings).toBe(true);
    expect(result.warnings.some((w) => w.message.includes('without expires_at'))).toBe(true);
  });
});
