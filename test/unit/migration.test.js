...
import { parseFrontmatter, formatFrontmatter } from '@context-vault/core/frontmatter';
...
describe('migration: guessKindFromPath', () => {
  ...
  it('extracts kind from directory name', () => {
    expect(guessKindFromPath('/vault/knowledge/decisions/my-decision.md', '/vault')).toBe(
      'decision'
    );
...
describe('migration: slug generation', () => {
  ...
  it('strips special characters from slugs', () => {
    expect(generateSlug('Hello, World! @#$%', '', '01ABCDEFGH')).toBe('hello-world-abcdefgh.md');
  });
});
