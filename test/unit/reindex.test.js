...
function writeMdFile(
  ...
  kindDir,
  filename,
  { id, kind, title, body, tags, source, created, identity_key, expires_at, meta }
) {
...
describe('reindex', () => {
  ...
  it('indexes new .md files from vault directory', async () => {
    writeMdFile(ctx.config.vaultDir, 'knowledge', 'insight', 'test-basic-01234567.md', {
      id: 'REINDEX_BASIC_01',
      kind: 'insight',
      title: null,
  ...
  it('returns correct stats object shape', async () => {
    writeMdFile(ctx.config.vaultDir, 'knowledge', 'decision', 'stats-test-01234567.md', {
      id: 'REINDEX_STATS_01',
      kind: 'decision',
  ...
  it('discovers nested kind directories (category/kind)', async () => {
    writeMdFile(ctx.config.vaultDir, 'events', 'session', 'nested-test-01234567.md', {
  ...
  it('skips files without frontmatter', async () => {
    ...
    const rows = ctx.db.prepare('SELECT id FROM vault WHERE file_path = ?').all(filePath);
...
describe('reindex — expired entry pruning', () => {
  ...
  it('prunes entries with past expires_at', async () => {
    ...
    writeMdFile(ctx.config.vaultDir, 'knowledge', 'insights', 'expired-test-01234567.md', {
