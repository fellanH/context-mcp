...
describe('full setup flow E2E', () => {
  ...
  it('completes setup --yes --skip-embeddings in an isolated HOME', () => {
    const { stdout, stderr, exitCode } = runCli('setup --yes --skip-embeddings', {
      env: { HOME: tmpHome },
      timeout: 60000,
    });
...
describe('setup --vault-dir flag', () => {
  it('uses custom vault dir from --vault-dir flag', () => {
    ...
    try {
      const { exitCode } = runCli(`setup --yes --skip-embeddings --vault-dir ${customVault}`, {
        env: { HOME: tmpHome },
        timeout: 60000,
      });
      expect(exitCode).toBe(0);
...
describe('setup error cases', () => {
  it('exits with error when vault dir path is a file', () => {
    ...
    try {
      const { exitCode, stderr, stdout } = runCli('setup --yes --skip-embeddings', {
        env: { HOME: tmpHome },
        timeout: 60000,
      });
      expect(exitCode).toBe(1);
...
describe('configureJsonTool via real CLI', () => {
  it('writes ~/.cursor/mcp.json when Cursor dir is detected', () => {
    ...
    try {
      ...
      // When context-vault is on PATH, uses the binary name; otherwise process.execPath
      expect(['context-vault', process.execPath]).toContain(entry.command);
  ...
  it('includes --vault-dir in tool config args when using custom vault', () => {
    ...
    try {
      ...
      const config = JSON.parse(readFileSync(join(tmpHome, '.cursor', 'mcp.json'), 'utf-8'));
  ...
  it('preserves other MCP server entries when adding context-vault', () => {
    ...
    writeFileSync(
      join(cursorDir, 'mcp.json'),
      JSON.stringify({ mcpServers: { 'other-mcp': { command: 'other', args: [] } } }, null, 2) +
        '\n'
    );
  ...
  it('detects Windsurf old-style dir (~/.codeium/windsurf) and writes mcp_config.json', () => {
    ...
    try {
      ...
      const configPath = join(tmpHome, '.codeium', 'windsurf', 'mcp_config.json');
      expect(existsSync(configPath)).toBe(true);
...
describe('setup config overwrite protection', () => {
  ...
  it('allows overwrite in --yes mode with --force flag', () => {
    ...
    try {
      const { exitCode } = runCli(`setup --yes --force --skip-embeddings --vault-dir ${newVault}`, {
        env: { HOME: tmpHome },
        timeout: 60000,
      });
      expect(exitCode).toBe(0);
